...
def getIndividualFeatureResult(all_X, global_y, global_train_index):
    ...
    clf = XGBClassifier(learning_rate=0.18, n_estimators=20, max_depth=4, objective='binary:logistic',
                        min_child_weight=5, gamma=5, subsample=0.5, colsample_bytree=0.5, metric='auc')
...
if __name__ == '__main__':
    ...
    for feature in all_features:
        if feature != 'Pssm':
            X = np.loadtxt(feature + 'Feature1.txt')
            all_X.append(X)
        else:
    ...
    classifier = XGBClassifier(learning_rate=0.2, n_estimators=20, max_depth=4, objective='binary:logistic',
                               metric='auc')
    ...
    results.to_csv('EnsembleLearningResults1.csv', index=False)
